...
onMounted(() => {
  ...
  const displayShaderSource = `
      ...
      void main () {
          ...
          if (maxC > 1.0) c /= maxC;
          float a = maxC > 0.0 ? min(maxC, 1.0) : 0.0;
  ...
  function initFramebuffers() {
    ...
    const texType = ext.halfFloatTexType || 0;
  ...
  function generateColor() {
    // HSV to RGB with random hue, full saturation and value — matches toukoum.fr
    const h = Math.random();
    const s = 1.0;
    const v = 1.0;
    ...
    // Scale down for subtle fluid effect
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    if (canvas.width / canvas.height < 1)
      pointer.deltaX *= canvas.width / canvas.height;
    else pointer.deltaY /= canvas.width / canvas.height;
  ...
  window.addEventListener("touchstart", (e) => {
    ...
    for (let i = 0; i < touches.length; i++) {
      ...
      pointer.down = true;
      pointer.texcoordX =
        scaleByPixelRatio(touches[i]?.clientX || 0) / canvas.width;
      pointer.texcoordY =
        1.0 - scaleByPixelRatio(touches[i]?.clientY || 0) / canvas.height;
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.color = generateColor();
    }
